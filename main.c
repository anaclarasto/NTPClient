...
int main( int argc, char* argv[ ] )
{
  ...
  char* host_name2 = "br.pool.ntp.org"; //NTP server 2 host-name
  ...
  *( ( char * ) &packet + 0 ) = 0x1b; // Represents 27 in base 10 or 00011011 in base 2.
  ...
  if ( server == NULL ){
      server = gethostbyname( host_name2 ); //Se der problema no primeiro servidor, testa o segundo
      if ( server == NULL){
  ...
  time_t txTmBrazil = (time_t)( packet.txTm_s - NTP_TIMESTAMP_DELTA -3*3600); //GMT Brasil -3:00
  time_t txTmPaloAlto = (time_t)( packet.txTm_s - NTP_TIMESTAMP_DELTA -7*3600); //GMT Palo Alto -7:00
